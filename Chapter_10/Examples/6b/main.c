// Example 10.6b	:  workqueue
//		:  main.c
//  

// select shared+fast or non-shared
// #define NONSHARED_SLOW_IRQHANDLER
#define SHARED_FAST_IRQHANDLER
...
#ifdef NONSHARED_SLOW_IRQHANDLER
	#define thisIRQ 13
	#define MYIRQFLAGS 0		// not shared */
#endif

// .. note: compiler warning here. but, o.k. for our learning.
#ifdef SHARED_FAST_IRQHANDLER
	#define thisIRQ 12
	#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
		#define MYIRQFLAGS IRQF_SHARED|IRQF_DISABLED // shared,fast */
	#else
		#define MYIRQFLAGS SA_SHIRQ | SA_INTERRUPT // shared, fast */
	#endif
#endif
...
irqreturn_t irq_handler(int irq, void *dev_id,struct pt_regs *regs)
{
    ...
    printk(KERN_ALERT "2470:10.6b: workqueue scheduled!\n");
    return IRQ_HANDLED;
}
    ...
    // #endif
   	...
   	if ((rc=(request_irq(thisIRQ,
        	(irq_handler_t) irq_handler,
       		...
       		(void*) &dev_id)))<0) {
   		printk(KERN_ALERT "2470:10.6b: intr not initialized! (RC=%d)\n",rc);
			goto err;
	}
	else {
   		printk(KERN_ALERT "2470:10.6b: intr initialized!\n");
	}

    // module init message
    printk(KERN_ALERT "2470:10.6b: main initialized!\n");
    printk(KERN_ALERT "2470:10.6b: memory allocated(hello_data) = %0x(%d bytes)!\n", (int) hello_data, ksize(hello_data));
    printk(KERN_ALERT "2470:10.6b: memory allocated(hello_data->wk) = %0x(%d bytes)!\n", (int) &hello_data->wk, ksize(hello_data->wk));
    printk(KERN_ALERT "2470:10.6b: memory allocated(hello_data->proc_hello_value) = %d!\n", ksize(hello_data->proc_hello_value));
...
err:
	...
	while (delayed_work_pending(hello_data->wk)) {
        ...
        printk(KERN_ALERT "2470:10.6b: Cancelling delayed work!\n");
	}
    ...
    // module exit message
  printk(KERN_ALERT "2470:10.6b: main bailing out!\n");
...
static void my_exit (void) {
    ...
    // module exit message
    printk(KERN_ALERT "2470:10.6b: main destroyed!\n");
}
