// Example 10.3b:  kernel timers+mod_timer+del_timer
//		:  main.c
...
#endif
{
	...
	if (timer_pending(usrsp->tlp)) {
        	//  print message
        	printk(KERN_ALERT "2470:10.3b: deleted timer!\n");
		// prefer to use del_timer_sync() 
		//	over del_timer - since del_timer_sync avoids
		//	race condition in SMP kernels.
		//	it is the same as del_timer in UP kernels.
		del_timer_sync(usrsp->tlp);
	}
	else {
        	printk(KERN_ALERT "2470:10.3b: did not delete any timer!\n");
	}

	// mod_timer can be called on inactive timers too,
	// i.e. in places where add_timer is called

       	printk(KERN_ALERT "2470:10.3b: add/mod timer!\n");
...
static int my_init (void) {
  ...
  proc_hello->owner = THIS_MODULE;
	...
	init_timer(hello_data->tlp);
        ...
        // module init message
        printk(KERN_ALERT "2470:10.3b: main initialized!\n");
        printk(KERN_ALERT "2470:10.3b: memory allocated(hello_data) = %d!\n", ksize(hello_data));
...
static void my_exit (void) {
	...
	if (proc_mydev)
        ...
        // module exit message
        printk(KERN_ALERT "2470:10.3b: main destroyed!\n");
}
