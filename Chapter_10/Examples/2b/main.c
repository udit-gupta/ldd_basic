// Example 10.2a: manual sleep+waitqueue
//				: prepare_to_sleep_exclusive()
//		:  main.c
//		remember the final read() call from cat cmd
...
#endif
{
	...
	*eof = 1;
	...
	printk(KERN_ALERT "2470:10.2a process(%d \"%s\") "
  ...
  // sleep_on and variants are removed - at this time
...
#endif
{
	...
	printk(KERN_ALERT "2470:10.2a: copying message!\n");
	if (copy_from_user(usrsp->proc_hello_value, buf, length)) 
		return -EFAULT;

	printk(KERN_ALERT "2470:10.2a: writing message!\n");
	
	usrsp->proc_hello_value[length-1]=0;
	usrsp->proc_hello_flag=1;

	// what happens when the following if cond (alone) is commented.
	printk(KERN_ALERT "2470:10.2a: check waitqueue active!\n");
	if (waitqueue_active(usrsp->proc_hello_wqh)) {
		printk(KERN_ALERT "2470:10.2a: issue .. wakeup (interruptible)!\n");
    wake_up_interruptible(usrsp->proc_hello_wqh);
		printk(KERN_ALERT "2470:10.2a: after issuing .. wakeup (interruptible)!\n");
	}
...
static int my_init (void) {
  ...
  proc_hello->write_proc = write_hello;
	...
	hello_data->proc_hello_flag=0;
        ...
        // module init message
        printk(KERN_ALERT "2470:10.2a: main initialized!\n");
	return 0;
}
...
static void my_exit (void) {
	...
	if (proc_mydev)
        ...
        // module exit message
        printk(KERN_ALERT "2470:10.2a: main destroyed!\n");
}
