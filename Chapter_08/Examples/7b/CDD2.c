...
{
		int thisPROC=(int) file->private_data;
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
static ssize_t writeproc_CDD2(struct file *file, const char __user *buf,
  size_t count, loff_t *ppos)
{
		int thisPROC=(int) file->private_data;
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
  	proc_CDD2[i] = proc_create("hello", 0777, proc_myCDD, &proc_fops);
    proc_CDD2[i]->file->private_data = (void *)i;
#else
