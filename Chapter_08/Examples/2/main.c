...
static int my_init (void) {
  ...
  proc_hello->write_proc = write_hello;
	...
	// memory definition
	hello_data=(struct proc_hello_data *)vmalloc(sizeof(*hello_data));
	...
	hello_data->proc_hello_flag=0;
        ...
        // module init message
        printk(KERN_ALERT "2470:8.2: main initialized!\n");
        printk(KERN_ALERT "2470:8.2: memory allocated(hello_data)=%d!\n", sizeof(*hello_data));  // what does ksize() here do? datatype of ksize
	return 0;
}
...
static void my_exit (void) {
  ...
  // module exit message
  printk(KERN_ALERT "2470:8.2: main destroyed!\n");
