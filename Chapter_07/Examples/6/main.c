// sourced from http://download.intel.com/embedded/software/IA/324264.pdf
...
void inline Filltimes(uint64_t **times) 
{
	...
	for (j=0; j<BOUND_OF_LOOP; j++) 
	{
		for (i =0; i<SIZE_OF_STAT; i++) 
		{
			...
			if ( (end - start) < 0) 
			{
				printk(KERN_ERR "%s #%d stat=%d st=%llu,end=%llu,variable=%u\n", 
					CRITICALERROR, j, i, start, end, variable);
...
#ifdef CONFIG_X86_32
	{
		long long rem, tmpvar=(temp_var - acc);
		rem=do_div(tmpvar,size*size);
		temp_var=tmpvar;
	}
#else
...
static int __init hello_start(void)
{
	...
	uint64_t *min_values;
	uint64_t max_dev = 0, 
						min_time = 0, max_time = 0, prev_min =0, tot_var=0,    
	...
	for (j=0; j<BOUND_OF_LOOP; j++) 
	{
		...
		printk(KERN_ERR "#%d >>>> variance(cycles): %llu; "
	...
	printk(KERN_ERR "\n total number of spurious min values = %d\n", spurious);
#ifdef CONFIG_X86_32
	do_div(tot_var,BOUND_OF_LOOP);
	printk(KERN_ERR "\n total variance = %llu\n",tot_var);
#else
