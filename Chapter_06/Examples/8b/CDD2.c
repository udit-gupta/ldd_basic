...
static struct proc_dir_entry *proc_myCDD[CDDNUMDEVS];
...
#define	PROCNAME file->f_path.dentry->d_name.name
    ...
    int len, int *eof, void *unused)
{
  ...
  struct CDD_proc *usrsp=&(CDD_procdata[i]);
...
static int CDD_init(void)
{
  ...
  proc_dirCDD = proc_mkdir(myCDD,0);
	...
	for (i=0;i<CDDNUMDEVS; i++) {
		...
		sprintf(procfs_name,"%s%d","hello",i);
		strncpy(CDD_procdata[i].CDD_procname,procfs_name,CDD_PROCLEN);
		
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
  	proc_myCDD[i] = proc_create(procfs_name, 0777, proc_dirCDD, &proc_fops);
    // proc_myCDD[i]->data = (void *)i;
#else
  	proc_myCDD[i] = create_proc_entry(procfs_name,0,proc_myCDD);
  	proc_myCDD[i]->read_proc = read_hello;
  	proc_myCDD[i]->write_proc = write_hello;
    proc_myCDD[i]->data = (void *)i;
#endif
...
static void CDD_exit(void)
{
	...
	for (i=0;i<CDDNUMDEVS; i++) {
		sprintf(procfs_name,"%s%d","hello",i);
  	if (proc_myCDD[i]) remove_proc_entry (procfs_name, proc_dirCDD);
	}
  if (proc_dirCDD) remove_proc_entry (procfs_name, proc_dirCDD);
