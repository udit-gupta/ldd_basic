...
static int CDD_init(void)
{
	...
	// quick initialize
	while(k<CDDNUMDEVS) { CDD_dev[k++].minor=-1; }
	...
	if ((errno=init_alloc_devices(CDDparm,0))!=0) 
		return -errno;
	if ((errno=init_alloc_devices(CDDparm,CDDGAP))!=0) 
	{
		deinit_unalloc_devices(CDDMajor,0);
		return -errno;
	}
...
static int deinit_unalloc_devices(int CDDmajor, int j)
{
	...
	i=0;
	thisCDD=&(CDD_dev[i]);
	while((thisCDD->minor<j)&&(i<CDDNUMDEVS)) 
			thisCDD=&(CDD_dev[i++]);
