...
static int CDD_init(void)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
  proc_CDD = proc_create("hello", 0777, proc_myCDD, &proc_fops);
#else 
  proc_CDD = create_proc_entry(HELLO,0,proc_myCDD);
  ...
  proc_CDD->write_proc = write_hello;
	...
	oroc_CDD2->write_proc = writeproc_CDD;
...
static void CDD_exit(void)
{
  ...
  if (proc_CDD) remove_proc_entry (CDD, proc_myCDD);
