...
static int CDD_init(void)
{
  ...
  dev_t devno;
    ...
    errno = register_chrdev_region(devno,CDDNUMDEVS,CDD);
    if (errno<0) { printk(KERN_ALERT "Error (%d) adding CDD",errno); 
			return errno;
		}
	}
	else {
    //  Step 1c of 2:  Request a Major Number Dynamically.
    errno = alloc_chrdev_region(&devno, CDDMINOR, CDDNUMDEVS, CDD);
    if (errno < 0) { printk(KERN_ALERT "Error (%d) adding CDD", errno); 
			return errno;
		}
...

  {
	...
	if (!CDDdevlist[minor].name) continue;
				...
				thisCDD->CDD_value=vmalloc(1024);
				thisCDD->CDD_devno = devno = MKDEV(CDDmajor,minor);
...

  {
    ...
    for (minor = 0; minor < ARRAY_SIZE(CDDdevlist); minor++) {
      if (!CDDdevlist[minor].name) continue;
			...
			//  Step 1 of 2:  del device from kernel
    	cdev_del(cdevp);
