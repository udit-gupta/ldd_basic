...
static struct CDDdev_struct CDD_dev[CDDGAP+CDDNUMDEVS+1];
...
static int CDD_init(void)
{
	...
	int k=0;
 	...
 	// quick initialize
	while(k<CDDNUMDEVS+CDDGAP) { CDD_dev[k++].minor=-1; }

	{
		if (CDDmajor) {
     	...
     	errno = register_chrdev_region(firstdevno,CDDNUMDEVS+CDDGAP,CDD);
     	if (errno < 0) { printk(KERN_ALERT "Error (%d) adding CDD", errno); return errno;}
		}
		else {
     	//  Step 1c of 2:  Request a Major Number Dynamically.
      errno = alloc_chrdev_region(&firstdevno, CDDMINOR, CDDNUMDEVS+CDDGAP, CDD);
     	if (errno < 0) { printk(KERN_ALERT "Error (%d) adding CDD", errno); return errno;}
		}
	...
	if ((errno=init_alloc_devices(CDDmajor,0))!=0) return -errno;
	if ((errno=init_alloc_devices(CDDmajor,CDDGAP))!=0) return -errno;
  ...
  proc_myCDD->owner = THIS_MODULE;
	...
	CDD_procdata.CDD_procvalue=vmalloc(1024);
	return 0;
}
...
static int deinit_unalloc_devices(int CDDmajor, int j)
{
	...
	for (i=j;i<CDDNUMDEVS+j; i++) {
		...
		if (thisCDD->minor==-1) continue;  // unused or dealloc'ed
		else thisCDD->minor=-1;
...
static void CDD_exit(void)
{
  ...
  if (proc_dirCDD) remove_proc_entry (myCDD, 0);
...

	{
  	...
  	unregister_chrdev_region(firstdevno, CDDNUMDEVS+CDDGAP);
